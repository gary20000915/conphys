"""
This is the homework for the Angry Birds with bisection method utilizing definite angle theta. 
Try to use bisection method to find out the velocity v0.
...
def Game(path) -> None:
    ...
    print("(One possible answer is [31, 32])")
...
def bi(a, b, pos0, t, theta=45) -> tuple:
    """
    ...
    Here we use the initial angle theta = 45.

    :param a: the beginning interval.
    :param b: the end interval.
    :param pos0: the initial position of angry bird.
    :param t: the time.
    :param theta: the angle we assigned as 45 [degrees].
    """
    ...
    while 0.5 * (b - a) > err:
        x = 0.5 * (a + b)
        i += 1
        pos = cla(pos0, x, theta, t)
        pos_temp = cla(pos0, a, theta, t)
...
def judge(path: str, pig: np.ndarray, x0: float, y0: float, a: float, b: float) -> None:
    ...
    while True:
        """v0 is the gaussed velocity and i is the iteration number."""
        v0, i = bi(a, b, pos0, t, theta)[0], bi(a, b, pos0, t, theta)[1]
...
if __name__ == "__main__":
    path = "trajectory_bisection_velocity"
    Game(path)
    # ans: [a, b] = [31, 32]
