...
def Game(path: str) -> None:
    ...
    while True:
        ...
        judge(path, pig, x0, y0, v0, theta)
        o.plot(path, True)


def judge(
    path: str, pig: np.ndarray, x0: float, y0: float, v0: float, theta: float
) -> None:
    """
    This is the judge function.

    :param path: name the file name.
    :param pig: the position of the pig.
    ...
    while True:
        ...
        if pos[1] < pig[1] or pos[0] > pig[0]:
            ...
            print("[dx, dy]:", f"[{(pos - pig)[0]:.3f}, {(pos - pig)[1]:.3f}] \n")
            break
        if t > 1000:
